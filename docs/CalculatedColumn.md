...
|  contains | Returns true if the Column mentioned contains the followed ‘Sub-String’, else it returns false | contains(if the given reference value has matched in the column mentioned it returns True or else false) |
...
Now you want to look at the quantity_sum difference by each month for each customer then select customer_id,add pivot to  month.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNTAzNTU5NDBdfQ==
-->